...
select
  base.*,
  greatest(0, current_date - coalesce(base.nomination_received_on, base.appointment_created_at::date))::integer as days_since_nomination,
  case
  ...
  case
    when base.ada_created_on is not null then 'SI'
    when base.ada_not_required then 'NO'
    when base.ada_sent_on is not null then 'SI'
    else 'PENDIENTE'
  end as ada_status,
  case
    ...
    when current_date - base.departure_date >= 11 then 'URGENTE'
    when current_date - base.departure_date between 6 and 10 then 'ATENCION'
    else 'OK'
  end as ada_priority,
  case
  ...
  case
    ...
    when base.departure_date is null then 'OK'
    when current_date - base.departure_date >= 45 then 'URGENTE'
    when current_date - base.departure_date between 30 and 44 then 'ATENCION'
    else 'OK'
  end as fda_priority
from base;
