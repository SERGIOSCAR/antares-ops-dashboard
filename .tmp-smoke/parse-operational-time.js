...
exports.parseOperationalInput = parseOperationalInput;
exports.toOperationalIso = toOperationalIso;
const OPERATIONAL_TIME_RE = /^(\d{1,2})\s?(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\s+((?:[01]?\d|2[0-3])(?:H|HS|HRS)?|(?:[01]?\d|2[0-3]):[0-5]\d|AM|PM|NOON|EAM|EPM|LPM)$/;
const monthMap = {
    JAN: 0,
    FEB: 1,
    MAR: 2,
...
function parseOperationalInput(raw) {
    ...
    const parsed = isPeriodToken ? { type: "period", value: token } : { type: "hour", value: token };
    return { day, monthCode, token, minuteProvided, isPeriodToken, periodDisplay, parsed };
}
function toOperationalIso(parsed, reference = new Date()) {
    ...
    if (dt.getMonth() !== month || dt.getDate() !== parsed.day)
        return null;
