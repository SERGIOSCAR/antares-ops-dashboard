...
import type { AppointmentRecipient, CreateAppointmentInput } from "@/lib/vesselmanager/types";
...
async function seedInitialTimeline(
  supabase: Awaited<ReturnType<typeof supabaseServer>>,
  appointmentId: string,
  appointmentDateTime?: string | null,
) {
  const eta = appointmentDateTime || null;
  if (!eta) return;
  const eventDate = eta.slice(0, 10);
  const withOperationalCols = await supabase.from("appointment_timeline").insert({
    appointment_id: appointmentId,
    event_type: "ETA_OUTER_ROADS",
    eta,
    ata: null,
  ...
  if (!withOperationalCols.error) return;

  if (
    withOperationalCols.error.message.includes("event_date") ||
    withOperationalCols.error.message.includes("event_time_text")
  ) {
    await supabase.from("appointment_timeline").insert({
      appointment_id: appointmentId,
      event_type: "ETA_OUTER_ROADS",
      eta,
      ata: null,
...
export async function POST(req: Request) {
  try {
    ...
    if (insertRes.error && hasMissingColumnError(insertRes.error.message)) {
      ...
      await seedInitialTimeline(supabase, fallbackInsert.data.id, payload.appointment_datetime);
    ...
    await seedInitialTimeline(
      supabase,
      insertRes.data.id,
      (insertRes.data.appointment_datetime as string | null | undefined) ?? payload.appointment_datetime,
    );
